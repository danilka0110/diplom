	...
	$query = R::getAll("SELECT q.question, q.survey_id, a.id, a.answer, a.question_id
		...
		LEFT JOIN survey
			ON survey.id = q.survey_id
				WHERE q.survey_id = $survey_id AND survey.enable = '0'");
	$survey_data = null;
    foreach($query as $item) {
        if( !$item['question_id'] ) return false;
		$survey_data[$item['question_id']][0] = $item['question'];
		$survey_data[$item['question_id']][$item['id']] = $item['answer'];
    }


	$query = R::getAll("SELECT s.survey_name, s.description, s.img_link, s.author
	FROM survey s
		WHERE s.id = $survey_id AND s.enable = '0'");
...
if (isset($_POST['btn-success'])) {
    ...
    $find_survey = R::findOne('survey', 'id = ?',[$survey_id]);
    $find_survey->enable = '1';
    R::store($find_survey);
            ...
            <div class="main-body">
                ...
                <div class="test-head">
                        <div class="text-center">
                            ...
                            <p style="font-size: 32px; color: #000;" class="test-head-test-name mt-4"><?=$survey_name?></p>
                        </div>

                        <div class="test-description mt-5">
                            <i class="desc-title">Описание теста: </i><span class="desc-text"><?=$survey_description?></span>
                            <hr>
                ...
                <div class="buttons">
                    <span><a class="btn btn-success" data-bs-toggle="modal" data-bs-target="#btn-success">Принять опрос</a></span>

                    <span><a class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#btn-delete">Удалить опрос</a></span>
                </div>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Открытие опроса</h5>
                            ...
                            <form action="survey" method="POST">
                                <div class="modal-body">
                                    ...
                                    <div class="input-group mb-3">
                                        ...
                                        <input type="text" class="form-control" name="survey_id" placeholder="ID" value="<?=$survey_id?>">
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Закрыть</button>
                                    <button class="btn btn-success" name="btn-success">Принять опрос</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <form action="survey" method="POST">
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Закрыть</button>
                                    <button class="btn btn-danger" name="btn-delete">Удалить опрос</button>
                                </div>
                            </form>
                        </div>
                    </div>

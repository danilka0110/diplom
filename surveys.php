<?php 
    ...
    ob_start();
    $surveys = get_surveys();
    $popular_surveys = get_popular_surveys();
    ...
    if(isset($_GET['category'])) {
        if ($_GET['category'] != NULL) {
          ...
          if (!in_array("$category_data", $categories)) {
            ...
            $category_surveys = get_surveys_for_category($category_data);
        } else {
            header('Location: surveys');
        }
      }
    ...
    if(isset($_POST['search'])) {
      if ($_POST['search'] != NULL) {
        ...
        $surveys_search = get_surveys_for_search($search_data);
    ...
    <div class="container-fluid tests-container">
        <div class="row">
                <?if ($_POST['search'] == NULL && $_GET['category'] == NULL) :?>
                    ...
                    <div class="page-list" align="center">
                        <?php for ($p = 0; $p < $page_count; $p++) :?>
                            <?php if ($p == $_GET['page']):?>
                                <a href="surveys?page=<?=$p?>"><button class="btn btn-outline-primary active" id="<?=$p?>"><?=$p+1?></button></a>
                            <?php else:?>
                                <a href="surveys?page=<?=$p?>"><button class="btn btn-outline-primary page_btn" id="<?=$p?>"><?=$p+1?></button></a>
                            <?php endif;?>
                        <?php endfor ;?>
                    </div>
                ...
                <?php else :?>
                    ...
                    <?php if ($category_surveys) :?>
                    <?php for ($i = 0; $i < count($category_surveys); $i++):?>
